...
public class FriendshipServiceImpl implements FriendshipService {
    ...
    @Override
    public List<FriendshipDto> getFriendshipRequestSent() {
        Object principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        String username;
        if (principal instanceof UserDetails)
            username = ((UserDetails)principal).getUsername();
        ...
        List<Friendship> userList = friendshipRepository.getAllFriendshipRequestSent(id);
        List<FriendshipDto> friends = new ArrayList<FriendshipDto>();
        for(Friendship x : userList){
            Optional<User> temp;
            temp=userRepository.findById(x.getFriendshipId().getReceiverId());
            if(temp.isPresent())
                friends.add(new FriendshipDto(temp.get().getId(),temp.get().getUsername(),temp.get().getName(),temp.get().getAvatar()));
        }
        return friends;
